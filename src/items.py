...
import items_exceptions as mvc_exc
...
def create_item(name, price, quantity):
    ...
    if results:
        warnings.warn(mvc_exc.ItemAlreadyStored('"{}" already stored!'.format(name)))
    else:
...
def read_item(name):
    ...
    if myitems:
        ...
        warnings.warn(mvc_exc.ItemNotStored('Can\'t read "{}" because it\'s not stored'.format(name)))
...
def update_item(name, price, quantity):
    ...
    if idxs_items:
        ...
        warnings.warn(mvc_exc.ItemNotStored('Can\'t update "{}" because it\'s not stored'.format(name)))
...
def delete_item(name):
    ...
    if idxs_items:
        ...
        warnings.warn(mvc_exc.ItemNotStored('Can\'t delete "{}" because it\'s not stored'.format(name)))
